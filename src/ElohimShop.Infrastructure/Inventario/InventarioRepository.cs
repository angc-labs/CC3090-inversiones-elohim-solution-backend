...
public class InventarioRepository : IInventarioRepository
{
    ...
    public async Task<InventarioResponseDto> GetInventarioAsync(InventarioQuery query, CancellationToken cancellationToken)
    {
        ...
        // Compute estado based on business rule. There is no StockMinimo column in the current schema;
        // we default StockMinimo = 0. If in the future a column exists, update this logic accordingly.
        ...
        var productos = await productosQuery.ToListAsync(cancellationToken);

        // Resumen
        var totalProductos = await _dbContext.Productos.AsNoTracking().CountAsync(cancellationToken);
        var stockAgotado = await _dbContext.Productos.AsNoTracking().CountAsync(p => p.StockActual == 0, cancellationToken);
